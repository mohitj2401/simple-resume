        ...
        @foreach ($resume->projects() as $item)
            <div class="company-title">
                <strong>
                    ...
                    <span style="color:#1c033c;">{{ $item->title }}</span>
                </strong>
                |
                @foreach ($item->skills() as $skill)
                    <span style="font-size: 11pt;"> {{ $skill->name }}
                        @if ($loop->index + 1 != count($item->skills()))
                            ,
                        @endif
                    </span>
                @endforeach
                @if ($resume->show_duration == 1)
                    <span style="color:#371e77;line-break:normal"
                        class="float-right text-md">{{ $item->duration }}</span>
                @endif
            </div>
            <ul>
                @foreach ($item->pointers_array() as $pointer)
                    <li style="">{{ $pointer }}</li>
                @endforeach
